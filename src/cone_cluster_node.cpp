...
public:
    ConeClusterNode()
        ...
        // ★ 円モデル（RANSAC）用パラメータ ★
        this->declare_parameter("circle_dist_thresh", 0.05); // 5cm (円からこの距離までを許容)
        this->declare_parameter("circle_min_radius", 0.05);  // 5cm (コーンの最小半径)
        this->declare_parameter("circle_max_radius", 0.3);   // 30cm (コーンの最大半径)

        update_parameters();

        param_callback_handle_ = this->add_on_set_parameters_callback(
            std::bind(&ConeClusterNode::parameterCallback, this, std::placeholders::_1));
...
private:
        ...
        const std::vector<rclcpp::Parameter> &parameters)
    {
        ...
        for (const auto &param : parameters) {
            if (param.get_name() == "cluster_tolerance") cluster_tolerance_ = param.as_double();
            ...
            if (param.get_name() == "circle_dist_thresh") circle_dist_thresh_ = param.as_double();
            if (param.get_name() == "circle_min_radius") circle_min_radius_ = param.as_double();
            if (param.get_name() == "circle_max_radius") circle_max_radius_ = param.as_double();
        }
        return result;
    }
